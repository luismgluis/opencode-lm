...
export function UserManagementRoutes(): Hono {
  ...
  app.put("/password", async (c) => {
    const user: { id: string } | undefined = c.get("user")
  ...
  app.put("/:id/password", async (c) => {
    const actor: { role: string } | undefined = c.get("user")
  ...
  app.get("/", async (c) => {
    const user: { role: string } | undefined = c.get("user")
  ...
  app.delete("/:id", async (c) => {
    const user: { role: string; id: string } | undefined = c.get("user")
    if (!user || user.role !== "admin") return c.json({ error: "Forbidden" }, 403)
